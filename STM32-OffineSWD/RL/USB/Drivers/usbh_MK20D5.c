...
#define USBH_MK_SZ_MPOOL          0x00000234
...
static BOOL USBH_Pins_Config (BOOL on) {
  ...
  /* Enable PTC8 (overcurrent flag) and PTC9 (VBUS drive enable)              */
  SIM->SCGC5    |=  (1   << 11);        /* Enable Port C Clock                */
  PTC->PDDR     &= ~(1   <<  8);        /* PTC8 is input                      */
  PORTC->PCR[8]  =  (1UL <<  8);        /* PTC8 is GPIO pin, no pull-up/down  */
  PTC->PDDR     |=  (1   <<  9);        /* PTC9 is output                     */
...
BOOL USBH_Init (BOOL on) {
  ...
  if (on) {
    ...
    SIM->CLKDIV2  =   SIM_CLKDIV2_USBDIV(0)     ; /* USB clk = (PLL*1/1)      */
                                                  /*         = ( 48*1/1)=48   */
...
static BOOL USBH_Port_Power (BOOL on) {
  ...
  if (on) 
    PTC->PSOR = (1 << 9);               /* PTC9 = 1 -> Drive ENA high         */
  else 
    PTC->PCOR = (1 << 9);               /* PTC9 = 0 -> Drive ENA low          */
