...
static BOOL USBH_Pins_Config (BOOL on) {
  ...
  /* Enable PTE31 (overcurrent flag) and PTB11 (VBUS drive enable)            */
  SIM->SCGC5    |=  SIM_SCGC5_PORTB_MASK; /* Enable Port B Clock              */
  SIM->SCGC5    |=  SIM_SCGC5_PORTE_MASK; /* Enable Port E Clock              */
  PTE->PDDR     &= ~(1UL << 31);        /* PTE31 is input                     */
  PORTE->PCR[31] =  (1   <<  8);        /* PTE31 is GPIO pin, no pull-up/down */
  PTB->PDDR     |=  (1   << 11);        /* PTB11 is output                    */
  PORTB->PCR[11] =  (1   <<  8);        /* PTB11 is GPIO pin                  */
...
static BOOL USBH_Port_Power (BOOL on) {
  ...
  if (on) 
    PTB->PSOR = (1 << 11);              /* PTB11 = 1 -> Drive ENA high        */
  else 
    PTB->PCOR = (1 << 11);              /* PTB11 = 0 -> Drive ENA low         */
