 ...
 *------------------------------------------------------------------------------
 *      Name:    usbh_MK60F12.c
 *      Purpose: USB Full/Low-speed Host Freescale Kinetis MK60F120M Driver module
 *      Rev.:    V4.70
 *------------------------------------------------------------------------------
...
BOOL USBH_Init (BOOL on) {
  ...
  if (on) {
    ...
    SIM->SOPT2   &= ~(SIM_SOPT2_PLLFLLSEL_MASK |  /* Clear SOPT2 USB FS values*/ 
                      SIM_SOPT2_USBFSRC_MASK   | 
                      SIM_SOPT2_USBF_CLKSEL_MASK);
    SIM->SOPT2   |=   SIM_SOPT2_PLLFLLSEL(1)  |   /* PLL0 reference           */
                      SIM_SOPT2_USBFSRC(0)    |   /* MCGPLLCLK as CLKC source */
                      SIM_SOPT2_USBF_CLKSEL_MASK; /* USB frac as USB ref clock*/
    SIM->CLKDIV2 &= ~(SIM_CLKDIV2_USBFSDIV_MASK  |/* Clear CLKDIV2 FS values  */
                      SIM_CLKDIV2_USBFSFRAC_MASK);
    SIM->CLKDIV2 |=   SIM_CLKDIV2_USBFSDIV(4) |   /* USB clk = (PLL0/2)*2/5   */
                      SIM_CLKDIV2_USBFSFRAC_MASK; /*         = (240/2)*2/5=48 */
    SIM->SCGC4   |=   SIM_SCGC4_USBFS_MASK;       /* Enable USBFS clock       */
    ...
    USB0->USBTRC0|=   USB_USBTRC0_USBRESET_MASK;  /* Reset the USB controller */
    while (USB0->USBTRC0 & USB_USBTRC0_USBRESET_MASK);      /* Wait to finish */
    SIM->SCGC4   &=  ~SIM_SCGC4_USBFS_MASK;       /* Disable USBFS clock      */
    SIM->CLKDIV2 &= ~(SIM_CLKDIV2_USBFSDIV_MASK  |/* Clear CLKDIV2 FS values  */
                      SIM_CLKDIV2_USBFSFRAC_MASK);
